...
add_filter( 'bogo_terms_translation', function ( $items, $locale ) {
    ...
    foreach ( get_taxonomies( array(), 'objects' ) as $taxonomy ) {
        if ( in_array( $taxonomy->name, [ 'nav_menu', 'wp_theme', 'wp_pattern_category' ] ) ) {
            continue;
        }
        ...
        foreach ( $terms as $term ) {
            ...
            $items[] = array(
                    ...
                    'context'    => sprintf( __( 'Slug (%s: %s)', 'bogo' ), $post_type->labels->singular_name,
...
add_action( 'pre_get_posts', function ( $query ) {
    if ( ! is_admin() && $query->is_main_query() ) {
        foreach ( get_taxonomies( [], 'objects' ) as $taxonomy ) {
            if ( ! $queried_term = $query->get( $taxonomy->name ) ) {
                continue;
            }
                ...
                <div class="wrap">
                    ...
                    <form method="post" action="options.php">
                        ...
                        <table class="wp-list-table widefat striped table-view-list">
                            ...
                            <thead>
                            <tr>
                                <th scope="col" id="context" class="manage-column column-context"><?php _e( 'Context',
                                            'bogo' ) ?></th>
                                <th scope="col" id="translation"
                                    class="manage-column column-translation"><?php _e( 'Translation', 'bogo' ) ?></th>
                                <th scope="col" id="original"
                                    class="manage-column column-original column-primary"><?php _e( 'Original' ) ?></th>
                            </tr>
                            </thead>
                            ...
                            <?php foreach ( $localizable_post_types = bogo_localizable_post_types() as $post_type ) {
                                ...
                                <tr>
                                    <td class="context column-context" data-colname="<?php _e( 'Context',
                                            'bogo' ) ?>"><?php echo $post_type->labels->singular_name ?></td>
                                    <td class="translation column-translation"
                                        data-colname="<?php _e( 'Translation', 'bogo' ) ?>">
                                            ...
                                            <?php foreach ( bogo_available_locales() as $locale ) {
                                                ...
                                                <div style="display: flex; align-items: center; gap: .42em">
                                                    ...
                                                    <label for="<?php echo $id ?>"
                                                           ...
                                                           title="<?php _e( $language ) ?>
                                                    "><?php _e( $locale ) ?></label>
                                                    <input type="text"
                                                           name="slugs[post-types][<?php echo $post_type->name ?>][<?php echo $locale ?>]"
                                                           id="<?php echo $id ?>" class="large-text"
                                                           value="<?php echo $slugs['post-types'][ $post_type->name ][ $locale ] ?? '' ?>"
                                                           placeholder="<?php echo $original ?>">
                                                </div>
                                            <?php } ?>
                                        </div>
                                    </td>
                                    <td class="original column-original column-primary"
                                        data-colname="<?php _e( 'Original', 'bogo' ) ?>">
                                        <?php echo $original ?>
                                    </td>
                                </tr>
                            ...
                            <tr>
                                <th colspan="3"><h2><?php _e( 'Archives' ) ?></h2></th>
                            </tr>

                            <?php foreach ( $localizable_post_types = bogo_localizable_post_types() as $post_type ) {
                                $post_type = get_post_type_object( $post_type );
                                if ( ! $post_type || $post_type->has_archive === false ) {
                                    continue;
                                }

                                $original = $post_type->has_archive;
                                $original = slug_decode( $original ); ?>
                                    ...
                                    <td class="translation column-translation"
                                        ...
                                        <div style="display: grid; gap: .5em;">
                                            <?php foreach ( bogo_available_locales() as $locale ) {
                                                $language = bogo_get_language( $locale ); ?>
                                                <div style="display: flex; align-items: center; gap: .42em">
                                                    <?php $id = "slug--archive--$post_type->name"; ?>
                                                    <label for="<?php echo $id ?>"
                                                           style="font-family: monospace;"
                                                           title="<?php _e( $language ) ?>
                                                    "><?php _e( $locale ) ?></label>
                                                    <input type="text"
                                                           name="slugs[archive][<?php echo $post_type->name ?>][<?php echo $locale ?>]"
                                                           id="<?php echo $id ?>" class="large-text"
                                                           value="<?php echo $slugs['archive'][ $post_type->name ][ $locale ] ?? '' ?>"
                                                           placeholder="<?php echo $original ?>">
                                                </div>
                                            <?php } ?>
                                        </div>
                            ...
                            <?php foreach (
                                ...
                                <tr>
                                    <td class="context column-context"
                                        data-colname="<?php _e( 'Context', 'bogo' ) ?>"><?php printf( '%s (%s)',
                                                $taxonomy->labels->singular_name, implode( ', ', $post_types ) ) ?></td>
                                    <td class="translation column-translation"
                                        data-colname="<?php _e( 'Translation', 'bogo' ) ?>">
                                        <div style="display: grid; gap: .5em;">
                                            <?php foreach ( bogo_available_locales() as $locale ) {
                                                $language = bogo_get_language( $locale ); ?>
                                                <div style="display: flex; align-items: center; gap: .42em">
                                                    <?php $id = "slug--taxonomy--$taxonomy->name"; ?>
                                                    <label for="<?php echo $id ?>" style="font-family: monospace;"
                                                           title="<?php _e( $language ) ?>"><?php _e( $locale ) ?></label>
                                                    <input type="text"
                                                           name="slugs[taxonomies][<?php echo $taxonomy->name ?>][<?php echo $locale ?>]"
                                                           id="<?php echo $id ?>" class="large-text"
                                                           value="<?php echo $slugs['taxonomies'][ $taxonomy->name ][ $locale ] ?? '' ?>"
                                                           placeholder="<?php echo $original ?>">
                                                </div>
                                            <?php } ?>
                                        </div>
                            ...
                            <tfoot>
                            <tr>
                                <th scope="col" class="manage-column column-context"><?php _e( 'Context',
                                            'bogo' ) ?></th>
                                <th scope="col" class="manage-column column-translation"><?php _e( 'Translation',
                                            'bogo' ) ?></th>
                                <th scope="col"
                                    class="manage-column column-original column-primary"><?php _e( 'Original',
                                            'bogo' ) ?></th>
                            </tr>
                            </tfoot>
...
add_filter( 'register_post_type_args', function ( $args, $post_type ) {
    if ( bogo_is_localizable_post_type( $post_type ) ) {
        ...
        if ( ( $args['has_archive'] ?? false ) !== false ) {
            if ( is_string( $args['has_archive'] ) ) {
                $slug = $args['has_archive'];
//            } elseif ( ( $args['rewrite'] ?? true ) !== false ) {
//                $slug = slug_decode( $args['rewrite']['slug'] );
            } else {
                $slug = $post_type;
            }

            $args['has_archive'] = slug_encode( $slug );
        }
    }
...
if ( ! function_exists( 'bogo_is_localizable_taxonomy' ) ) {
    ...
    function bogo_is_localizable_taxonomy( $taxonomy ): bool {
        ...
        return array_key_exists( $taxonomy->name ?? $taxonomy, $localizable_taxonomies );
    }
}
...
add_filter( 'rewrite_rules_array', function ( $rules ) {
    ...
    foreach ( $localizable_post_types = bogo_localizable_post_types() as $post_type ) {
        ...
        if ( $post_type->has_archive ) {
            // add default slug
            $slugs['archive'][ $post_type->name ][] = slug_decode( $post_type->has_archive );

            foreach ( $rules as $regex => $query ) {
                // look for encoded slug
                if ( str_contains( $regex, $post_type->has_archive ) ) {
                    // add all possible slugs
                    foreach ( array_filter( array_unique( $slugs['archive'][ $post_type->name ] ) ) as $locale => $translation ) {
                        $updated_regex = str_replace( $post_type->has_archive, $translation, $regex );
                        // normally not necessary, but just to be safe
                        $updated_query = str_replace( $post_type->has_archive, $translation, $query );
...
add_filter( 'post_type_archive_link', function ( $link, $post_type ) {
    if ( bogo_is_localizable_post_type( $post_type ) ) {
        ...
        if ( $post_type && $post_type->has_archive ) {
            // get post type specific archive slugs
            $slugs = get_option( 'slugs', [] )['archive'][ $post_type->name ] ?? [];

            if ( $slug = ( $slugs[ get_locale() ] ?? '' ) ?: slug_decode( $post_type->has_archive ) ) {
                $link = str_replace( $post_type->has_archive, $slug, $link );
            }
        }
    }
...
/**
 * Correct archive URL due to slug translation.
 */
add_filter( 'bogo_language_switcher_links', 'translate_archive_links', 10, 2 );
add_filter( 'bogo_rel_alternate_hreflang', 'translate_archive_links' );
function translate_archive_links( $links ): array {
    if ( is_post_type_archive() ) {
        if ( bogo_is_localizable_post_type( $post_type = get_query_var( 'post_type' ) ) ) {
            $post_type = get_post_type_object( $post_type );
            if ( $post_type && $post_type->has_archive ) {
                foreach ( $links as $i => $link ) {
                    $locale = str_replace( '-', '_', $link['hreflang'] ?? $link['locale'] );
                    if ( get_locale() == $locale ) {
                        continue;
                    }

                    switch_to_locale( $locale );
                    $links[ $i ]['href'] = get_post_type_archive_link( get_query_var( 'post_type' ) );
